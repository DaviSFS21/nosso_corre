...
<head>
    ...
    <link rel="stylesheet" href="../../pages/news/css/style.css">
    <title>Nosso Corre - Portal de notícias</title>
</head>

<body>
    <header class="header responsive-wrapper">
        <div class="header-left">
            <img src="../../assets/images/svg/Nosso_Corre.svg" alt="Logo" style="height: 120px; width: 120px;">
            <defs>
        ...
        <div class="header-middle">
            <a href="#" class="header-link"><b>Fale conosco: </b>nossocorre@gmail.com</a>
        </div>
        <div class="header-right">
            <nav class="header-nav">
                <a href="./news.php" class="header-link">Notícias - Início</a>
            </nav>
            <a href="../home/index.php" class="header-menu-button">Home</a>
        </div>
    </header>
    <main class="responsive-wrapper">
        <div class="page-title">
            <h1 style="font-family: Grafite;">Portal de notícias</h1><br>
        </div>
        <?php 
            if(isset($_GET['page'])) {
                if ($_GET['page'] == 0) {
        ...
        ?>
        <br><center><h2>Página <?php echo $pagina; ?></h2></center>
        <div class="magazine-layout" style="display:unset;">
            <div class="magazine-column">
            <?php
            /* Este trecho realiza a conexão com o banco de dados, executa uma pesquisa
            e armazena informações de algumas notícias em vetores. */
            ...
            $result_artigos = mysqli_query($conexao, "SELECT * FROM `artigos` 
            WHERE `usuario_id` = " . $_SESSION['id'] . " ORDER BY `data` DESC LIMIT 4 OFFSET $min");
            $numero_result = mysqli_num_rows($result_artigos);

            if($numero_result != 0){
                for($i = 1; $i <= $numero_result; $i++){
                    $vetor_artigos = mysqli_fetch_array($result_artigos);
                    $novaData = new DateTime($vetor_artigos[5]);
                    $data = $novaData->format('d/m/Y');

                    echo '<img src="' . $vetor_artigos[4] . '" />';
                    echo '<h2 class="article-title article-title--large"><a href="#" class="article-link">'
                     . $vetor_artigos[1] . '</a></h2>';
                    echo '<div class="article-excerpt" style="font-size: 24px;">' . $vetor_artigos[3] . '</div>';
                    echo '<div class="article-author"><div class="article-author-info"><dl><dt>'
                     . $vetor_artigos[2] . '</dt><dd>' . $data . '</dd></dl></div></div>';
                    echo "<a href='../../controllers/excluir_artigo.php?id_artigo=" . $vetor_artigos[0] . "&url_img=" . $vetor_artigos[4]. "'
                    onclick=\"return confirm('Tem certeza que deseja deletar este artigo?')\">Excluir artigo</a><br><br>";
                    echo "<a href='./newsedit.php?id_artigo=" . $vetor_artigos[0] . "'>Editar artigo</a><hr><br>";
                }
            }else{
                ?>
                    <script>
                        alert("Não há mais nada aqui...");
                        window.location.replace("?page=<?php if(isset($_GET['page'])) { echo $_GET['page'] - 1; } ?>");
                    </script>
                <?php
            }
        ?>
        ...
        <div class="crudnews">
            <a href="?page=<?php if(isset($_GET['page'])) { echo $_GET['page'] - 1; } else { echo "0"; } ?>">Anterior</a>
            <a href="?page=<?php if(isset($_GET['page'])) { echo $_GET['page'] + 1; } else { echo "1"; } ?>">Próxima</a>
        </div>
        <div class="crudnews">
            <h1>Sua vez</h1>
            <a href="newscrud.php">Escrever notícia</a>
        </div>
        <!-- News -->
        <!--  jQuery -->
