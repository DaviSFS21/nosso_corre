...
<body>
    <header class="header responsive-wrapper">
        ...
        <div class="header-right">
            <nav class="header-nav">
                <a href="./usernews.php#page=0" class="header-link">Minhas notícias</a>
            </nav>
    ...
    <main class="responsive-wrapper">
        <div class="page-title">
            <h1 style="font-family: Grafite;">Portal de notícias</h1>
        </div>
        <?php 
            if(isset($_GET['page'])) {
                if ($_GET['page'] == 0) {
        ...
        <?php
            ...
            $result_artigos = mysqli_query($conexao, "SELECT * FROM `artigos` ORDER BY `data` DESC LIMIT 7 OFFSET $min");
            $numero_result = mysqli_num_rows($result_artigos);

            if($numero_result != 0){
                for($i = 1; $i <= $numero_result; $i++){
                    $vetor_artigos = mysqli_fetch_array($result_artigos);
                    $imagem[$i] = $vetor_artigos[4];
                    $titulo[$i] = $vetor_artigos[1];
                    $autor[$i] = $vetor_artigos[2];
                    $texto[$i] = $vetor_artigos[3];
                    $novaData = new DateTime($vetor_artigos[5]);
                    $data[$i] = $novaData->format('d/m/Y');
                }
            }else{
                ?>
                    <script>
        ...
        <div class="magazine-layout">
            <div class="magazine-column">
                <article class="article"><?php $n += 1; ?>
                    <?php
                        if (isset($imagem[$n])) {
                            echo '<img src="' . $imagem[$n] . '" />';
                        }

                        if (isset($titulo[$n])) {
                            echo '<h2 class="article-title article-title--large"><a href="#" class="article-link">' . $titulo[$n] . '</a></h2>';
                        }

                        if (isset($texto[$n])) {
                            echo '<div class="article-excerpt">' . $texto[$n] . '</div>';
                        }

                        if (isset($autor[$n]) && isset($data[$n])) {
                            echo '<div class="article-author"><div class="article-author-info"><dl><dt>' . $autor[$n] . '</dt><dd>' . $data[$n] . '</dd></dl></div></div>';
                        }
                    ?>
                </article>
                <article class="article"><?php $n += 1; ?>
        ...
        <div class="crudnews">
            <a href="?page=<?php if(isset($_GET['page'])) { echo $_GET['page'] - 1; } else { echo "-1"; } ?>">Anterior</a>
